...
kable(cbind(row_name, rbind(combined_AVG, sapply(chance_10, function(x) paste('1 in', x)), sapply(chance_25, function(x) paste('1 in', x)))))
...
```{r cache=TRUE, echo=FALSE}
normality <- do.call(rbind, mclapply(years, mc.cores = ncores, FUN = function(xx){
  m <- batters_talent_AVG %>% filter(yearID == xx, full_time == 'Y')
  return(c(xx, shapiro.test(m$comp)$p.value))
}))

foo <- data.frame(p_value = c(0.05, 0.1, 0.2, 0.3), 
                  number_season = sapply(c(0.05, 0.1, 0.2, 0.3), function(x) sum(normality[,2] < x)/152))
kable(foo)
```
...
kable(cbind(row_name, rbind(BA_OBP_HR, chance_10, chance_25)))
